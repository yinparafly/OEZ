...
class C2kLink(SerialLink):
    """C2000 固件协议翻译器（双协议并存，UI 界面零改动）。

    与原始 SerialLink 同一接口（send / arm_bindump / is_open / out_q），
    内部完成两个方向的翻译：
      下行：UI 的 ESP32 命令 → C2000 CLI 命令（不支持的命令吞掉并回空响应）
      上行：C2000 的行 → UI 认识的 ESP32 事件格式（# ARMED→# MONITOR armed 等）
    BIN 拉取与原始协议兼容：固件 DUMP BIN 输出 v2 帧（preamble+magic+n+hz+点+crc）。
    """

    # ESP32 命令 → C2000 命令；None = 本固件不支持（吞掉，回 # CAP empty）
    _CMD_MAP = {
        "LOG CLEAR": None,
        "LOG DUMP": "DUMP BIN",
        "REC NOW": "REC NOW",
    ...
    _CMD_UNSUPPORTED = {"LOG CLEAR": "# LOG CLEARED", "CAPTURE START": "# CAP empty",
                        "CAPTURE STOP": "# CAP empty", "CAPTURE DUMP": "# CAP empty",
                        "CAPTURE NEXT": "# CAP empty", "CAPTURE?": "# CAP empty"}

    def send(self, cmd: str) -> None:
        c = cmd.strip().upper()
        if not c:
            return
        # 前缀匹配：CAPTURE START 30 → CAPTURE START
        key = c
        if key not in self._CMD_MAP:
            for k in self._CMD_MAP:
                if c.startswith(k):
                    key = k
                    break
        if key in self._CMD_MAP:
            t = self._CMD_MAP[key]
            if t is None:
                # 固件无长采/清空：回空响应让 UI 正常退出等待态
                self.out_q.put(("__raw__", self._CMD_UNSUPPORTED.get(key, "# CAP empty")))
                return
            c = t
        super().send(c)
    ...
    def _translate_line(self, line: str) -> str | None:
        """C2000 响应行 → UI 事件格式。返回 None 表示丢弃。"""
        if line.startswith("# ARMED"):
            return "# MONITOR armed"
        if line.startswith("# STOPPED"):
            return "# disarm"
        if line.startswith("# SNAP empty"):
            self.cancel_bindump()
            return "# LOG empty"
        if line.startswith("RPM=[") or line.startswith("= = =") or line.startswith("=== Welcome"):
            return None  # 主循环 1Hz 状态行/横幅，UI 不需要
        if line.startswith("# SNAP n=") or line.startswith("# SNAP "):
            return self._translate_snap_line(line)
        return line

    def _translate_snap_line(self, line: str) -> str:
        """# SNAP n=.. bytes=.. armed=.. rec=.. alive=.. done=.. → UI 事件。"""
        toks: dict[str, str] = {}
        for tok in line.replace("#", " ").split():
            if "=" in tok:
                k, _, v = tok.partition("=")
                toks[k] = v
        n = int(toks.get("n", "0") or 0)
        done = int(toks.get("done", "0") or 0)
        alive = int(toks.get("alive", "0") or 0)
        rec = int(toks.get("rec", "0") or 0)
        if done:
            return f"# SNAP DONE n={n}"
        if alive and not rec:
            return f"# SNAP DUMP READY n={n}"
        return f"# SNAP n={n} ring={n} hz=2000 ready=0 rec={rec} arm={int(toks.get('armed','0') or 0)} alive={alive}"
...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        if PREFER_SIM:
            ...
            self.var_link = tk.StringVar(value="OEZ-ABI (蓝牙)" if prefer_ble else "OEZ-ABI (USB)")
    ...
    def _build(self) -> None:
        ...
        top.pack(fill=tk.X)
        ttk.Label(top, text="设备型号").pack(side=tk.LEFT)
        ttk.Combobox(
            top,
            textvariable=self.var_link,
            values=("OEZ-ABI (USB)", "OEZ-ABI (蓝牙)", "仿真SIM", "TJX-C2000 (USB)"),
            width=16,
            state="readonly",
    ...
    def _is_ble_mode(self) -> bool:
        v = self.var_link.get()
        return "蓝牙" in v or "BLE" in v.upper()

    def _is_c2k_mode(self) -> bool:
        v = self.var_link.get()
        return "C2000" in v.upper()
    ...
    def _sync_link_ui(self) -> None:
        if self._is_sim_mode():
            ...
            if hasattr(self, "btn_dump"):
                self.btn_dump.configure(text="仅拉取(USB全速)" + (" · C2000" if self._is_c2k_mode() else ""))
    ...
    def _toggle_conn(self) -> None:
        ...
        if self._is_ble_mode():
            ...
            if not port:
                ...
                return
            if self._is_c2k_mode():
                self.link = C2kLink(port, self.out_q, self.stop_evt)
            else:
