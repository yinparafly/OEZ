# -*- coding: utf-8 -*-
"""高速阶梯：闭环平滑升到 8500RPM，稳定保持 ~20s 后安全停车。

- HOST OFF 防卡滞；MODE CLOSED + SOFT ON + RPMMAX 12000；目标 8500；FREQ 400。
- 先门闩 motor_spin_detect @2000；FAIL → ESTOP，禁止升 8500。
...
- 只做本档，不自动升 9000；汇报后问用户。

用法: PYTHONIOENCODING=utf-8 python _hi_step_8500.py
"""
...
TARGET = 8500.0
RPMMAX = 12000
SOFT_UP = 900          # rpm/s；2000→8500 ≈ 7.2s
SOFT_DOWN = 700
...
LOG = LOGDIR / f"hi_step_8500_{STAMP}.txt"
CSVP = LOGDIR / f"hi_step_8500_{STAMP}.csv"
JSONP = LOGDIR / f"hi_step_8500_{STAMP}.json"
...
def main():
    ...
    ff_9000 = interp_pulse_for_rpm(ff_points, 9000.0) if ff_points else None

    log(f"=== 高速阶梯 8500RPM  {datetime.now().isoformat(timespec='seconds')} ===")
    ...
    log(f"FF csv={ff_csv} points={len(ff_points)} ff_max={ff_max_rpm}RPM@{ff_max_pulse}us "
        f"ff_pulse@8500={ff_pulse} ff_pulse@9000(ref)={ff_9000}")
    if ff_max_rpm is not None and ff_max_rpm < TARGET:
        log(f"注意: 前馈图最高仅 {ff_max_rpm:.0f}RPM，8500 超出学习范围（PID 积分补偿），"
            f"ff_pulse 为末点钳位值")
    ...
    try:
        ...
        if not gate.spinning:
            log("SPIN_DETECT FAIL — ESTOP，禁止升 8500")
            estop(ser)
            safe_stop(ser)
            try:
                require_spinning(gate, label="hi_step_8500_gate")
        ...
        if mean_rpm == mean_rpm:
            log(f"静差 static_err={static_err:.1f} ({static_err/TARGET*100:.2f}%)  "
                f"达到8500并稳定={'是' if reached else '否'}")
        ...
        log(f"PID 输出 u=pulse-ff: ff_pulse@8500={ff_pulse} -> u={u}"
        ...
        cover_9000 = (ff_max_rpm is not None and ff_max_rpm >= 9000)
        suggest_9000 = bool(
        ...
        log(f"\n余量判断(是否可继续 9000): pulse 距 2000 约 {margin_2000:.0f}us; "
            f"前馈图覆盖9000={'是' if cover_9000 else '否(末点%s RPM)' % (int(ff_max_rpm) if ff_max_rpm else '?')}; "
            f"建议试9000={'是' if suggest_9000 else '否/谨慎'}")
        log("本档结束，不自动升 9000。请人工确认是否继续。")
        ...
        summary = {
            ...
            "ff_pulse_at_8500": ff_pulse,
            "ff_max_rpm": ff_max_rpm,
            "ff_max_pulse": ff_max_pulse,
            "ff_covers_9000": bool(cover_9000),
