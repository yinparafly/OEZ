# -*- coding: utf-8 -*-
"""高速阶梯：闭环平滑升到 9000RPM，稳定保持 ~20s 后安全停车。

- HOST OFF 防卡滞；MODE CLOSED + SOFT ON + RPMMAX 12000；目标 9000；FREQ 400。
- 架构：采集/使用 2kHz、输出 400Hz（勿改 1200）。
- 先门闩 motor_spin_detect @2000；FAIL → ESTOP，禁止升 9000。
...
用法: PYTHONIOENCODING=utf-8 python _hi_step_9000.py
"""
...
TARGET = 9000.0
RPMMAX = 12000
SOFT_UP = 900          # rpm/s；2000→9000 ≈ 7.8s
SOFT_DOWN = 700
...
LOG = LOGDIR / f"hi_step_9000_{STAMP}.txt"
CSVP = LOGDIR / f"hi_step_9000_{STAMP}.csv"
JSONP = LOGDIR / f"hi_step_9000_{STAMP}.json"
...
def main():
    ...
    ff_10000 = interp_pulse_for_rpm(ff_points, 10000.0) if ff_points else None

    log(f"=== 高速阶梯 9000RPM  {datetime.now().isoformat(timespec='seconds')} ===")
    ...
    log(f"FF csv={ff_csv} points={len(ff_points)} ff_max={ff_max_rpm}RPM@{ff_max_pulse}us "
        f"ff_pulse@9000={ff_pulse} ff_pulse@10000(ref)={ff_10000}")
    ...
    try:
        ...
        if not gate.spinning:
            log("SPIN_DETECT FAIL — ESTOP，禁止升 9000")
            estop(ser)
            safe_stop(ser)
            try:
                require_spinning(gate, label="hi_step_9000_gate")
        ...
        if mean_rpm == mean_rpm:
            log(f"静差 static_err={static_err:.1f} ({static_err/TARGET*100:.2f}%)  "
                f"达到9000并稳定={'是' if reached else '否'}")
        ...
        log(f"PID 输出 u=pulse-ff: ff_pulse@9000={ff_pulse} -> u={u}"
        ...
        cover_10000 = (ff_max_rpm is not None and ff_max_rpm >= 10000)
        suggest_10000 = bool(
        ...
        log(f"\n余量判断(是否可继续 10000): pulse 距 2000 约 {margin_2000:.0f}us; "
            f"前馈图覆盖10000={'是' if cover_10000 else '否(末点%s RPM)' % (int(ff_max_rpm) if ff_max_rpm else '?')}; "
            f"建议试10000={'是' if suggest_10000 else '否/谨慎'}")
        log("本档结束，不自动再升。请人工确认是否继续。")
        ...
        summary = {
            ...
            "ff_pulse_at_9000": ff_pulse,
            "ff_max_rpm": ff_max_rpm,
            "ff_max_pulse": ff_max_pulse,
            "ff_covers_10000": bool(cover_10000),
