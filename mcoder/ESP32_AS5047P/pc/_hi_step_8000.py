# -*- coding: utf-8 -*-
"""高速阶梯：闭环平滑升到 8000RPM，稳定保持 ~20s 后安全停车。

- HOST OFF 防卡滞；MODE CLOSED + SOFT ON + RPMMAX 12000；目标 8000；FREQ 400。
- 先门闩 motor_spin_detect @2000；FAIL → ESTOP，禁止升 8000。
...
- 只做本档，不自动升 8500；汇报后问用户。

用法: PYTHONIOENCODING=utf-8 python _hi_step_8000.py
"""
...
TARGET = 8000.0
RPMMAX = 12000
SOFT_UP = 900          # rpm/s；2000→8000 ≈ 6.7s
SOFT_DOWN = 700
...
LOG = LOGDIR / f"hi_step_8000_{STAMP}.txt"
CSVP = LOGDIR / f"hi_step_8000_{STAMP}.csv"
JSONP = LOGDIR / f"hi_step_8000_{STAMP}.json"
...
def main():
    ...
    ff_8500 = interp_pulse_for_rpm(ff_points, 8500.0) if ff_points else None

    log(f"=== 高速阶梯 8000RPM  {datetime.now().isoformat(timespec='seconds')} ===")
    ...
    log(f"FF csv={ff_csv} points={len(ff_points)} ff_max={ff_max_rpm}RPM@{ff_max_pulse}us "
        f"ff_pulse@8000={ff_pulse} ff_pulse@8500(ref)={ff_8500}")
    if ff_max_rpm is not None and ff_max_rpm < TARGET:
        log(f"注意: 前馈图最高仅 {ff_max_rpm:.0f}RPM，8000 超出学习范围（PID 积分补偿），"
            f"ff_pulse 为末点钳位值")
    ...
    try:
        ...
        if not gate.spinning:
            log("SPIN_DETECT FAIL — ESTOP，禁止升 8000")
            estop(ser)
            safe_stop(ser)
            try:
                require_spinning(gate, label="hi_step_8000_gate")
        ...
        if mean_rpm == mean_rpm:
            log(f"静差 static_err={static_err:.1f} ({static_err/TARGET*100:.2f}%)  "
                f"达到8000并稳定={'是' if reached else '否'}")
        ...
        log(f"PID 输出 u=pulse-ff: ff_pulse@8000={ff_pulse} -> u={u}"
        ...
        cover_8500 = (ff_max_rpm is not None and ff_max_rpm >= 8500)
        suggest_8500 = bool(
        ...
        log(f"\n余量判断(是否可继续 8500): pulse 距 2000 约 {margin_2000:.0f}us; "
            f"前馈图覆盖8500={'是' if cover_8500 else '否(末点%s RPM)' % (int(ff_max_rpm) if ff_max_rpm else '?')}; "
            f"建议试8500={'是' if suggest_8500 else '否/谨慎'}")
        log("本档结束，不自动升 8500。请人工确认是否继续。")
        ...
        summary = {
            ...
            "ff_pulse_at_8000": ff_pulse,
            "ff_max_rpm": ff_max_rpm,
            "ff_max_pulse": ff_max_pulse,
            "ff_covers_8500": bool(cover_8500),
