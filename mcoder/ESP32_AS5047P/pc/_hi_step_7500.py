# -*- coding: utf-8 -*-
"""高速阶梯：闭环平滑升到 7500RPM，稳定保持 ~20s 后安全停车。

- MODE CLOSED + SOFT ON + RPMMAX 12000；目标 7500RPM；FREQ 400。
- 先门闩 motor_spin_detect @2000；FAIL → ESTOP，禁止升 7500。
- 后台独立进程；每 0.5s 重发 RPM 保活（<HOST_TIMEOUT 1.5s）防 ESTOP。
- 稳态采：enc 均值/spread/超调、pulse、饱和、霍尔交叉、LOAD?(overrun/enc_busy)。
- 稳态中抽检 spin_detect；到时或异常 → 平滑降速 → RPM0/STOP/PWM1000；释放锁。
- 只做本档，不自动升 8000。

用法: PYTHONIOENCODING=utf-8 python _hi_step_7500.py
"""
...
TARGET = 7500.0
RPMMAX = 12000
SOFT_UP = 900          # rpm/s；0→7500 ≈ 8.3s，不在低速顿挫区停留
SOFT_DOWN = 700
SETTLE_S = 12.0        # 到该秒后计入稳态（升速 ~8.3s + 余量）
HOLD_S = 33.0          # 段总时长；稳态 [12,33] ≈ 21s ≥ 20s
KEEPALIVE_S = 0.5
POLL_S = 3.0
SAT_PULSE = 1985
SPIN_SPOT_AT = 20.0    # 稳态中途抽检 spin_detect 的段内时刻

LOGDIR = Path(__file__).resolve().parent / "logs"
STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
LOG = LOGDIR / f"hi_step_7500_{STAMP}.txt"
CSVP = LOGDIR / f"hi_step_7500_{STAMP}.csv"
JSONP = LOGDIR / f"hi_step_7500_{STAMP}.json"
    ...
    while time.time() < end:
        for ln in drain(ser, 0.15):
            if ln.startswith("# LOAD"):
                raw = ln
                d = parse_load(ln)
                # enc_ring_overrun 在 parse_load 的正则外，补抓
                if d is not None:
...
def main():
    ...
    ff_8000 = interp_pulse_for_rpm(ff_points, 8000.0) if ff_points else None

    log(f"=== 高速阶梯 7500RPM  {datetime.now().isoformat(timespec='seconds')} ===")
    ...
    log(f"FF csv={ff_csv} points={len(ff_points)} ff_max={ff_max_rpm}RPM@{ff_max_pulse}us "
        f"ff_pulse@7500={ff_pulse} ff_pulse@8000(clamped)={ff_8000}")
    if ff_max_rpm is not None and ff_max_rpm < TARGET:
        log(f"注意: 前馈图最高仅 {ff_max_rpm:.0f}RPM，7500 超出学习范围（PID 积分补偿），"
            f"ff_pulse 为末点钳位值")
    ...
    try:
        ...
        log("--- idle ENC? / LOAD RESET / LOAD? (电机停) ---")
        ...
        if not gate.spinning:
            log("SPIN_DETECT FAIL — ESTOP，禁止升 7500")
            estop(ser)
            safe_stop(ser)
            try:
                require_spinning(gate, label="hi_step_7500_gate")
        ...
        if mean_rpm == mean_rpm:
            log(f"静差 static_err={static_err:.1f} ({static_err/TARGET*100:.2f}%)  "
                f"达到7500并稳定={'是' if reached else '否'}")
        ...
        log(f"PID 输出 u=pulse-ff: ff_pulse@7500={ff_pulse} -> u={u}"
        ...
        cover_8000 = (ff_max_rpm is not None and ff_max_rpm >= 8000)
        suggest_8000 = bool(
            reached and not aborted and not saturated and load_ok
            and (margin_2000 == margin_2000 and margin_2000 >= 200)
            and (hall_err_pct != hall_err_pct or hall_err_pct < 5.0)
        )
        log(f"\n余量判断(是否可继续 8000): pulse 距 2000 约 {margin_2000:.0f}us; "
            f"前馈图覆盖8000={'是' if cover_8000 else '否(末点%s RPM)' % (int(ff_max_rpm) if ff_max_rpm else '?')}; "
            f"建议试8000={'是' if suggest_8000 else '否/谨慎'}")
        log("本档结束，不自动升 8000。请人工确认是否继续。")
        ...
        summary = {
            ...
            "ff_pulse_at_7500": ff_pulse,
            "ff_max_rpm": ff_max_rpm,
            "ff_max_pulse": ff_max_pulse,
            "ff_covers_8000": bool(cover_8000),
