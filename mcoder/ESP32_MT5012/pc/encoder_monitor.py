...
# -*- coding: utf-8 -*-
"""
TLE5012B (5012/MT5012) 磁编码器 PC 监视器
- 实时显示角度 0~360° / 0~2π，钟表式相位表盘（0° 在上方，顺时针）
- 开始/停止记录，绘制转速(RPM) vs 时间
- 可导出记录为 CSV
- 串口协议与 AS5047P 版兼容: t_ms,raw,deg,rad,rpm[,ef,agc,magL,magH]
"""
...
class EncoderMonitorApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("TLE5012B (5012) 磁编码器监视器")
    ...
    def _connect(self) -> None:
        ...
        self.status_var.set(
            f"已连接 {port} @ {BAUD} | 采样 {self.sample_hz}Hz → 控制 {self.ctrl_hz}Hz"
        )
        self.prev_deg = None
        self.prev_t_ms = None
        self._rx_count = 0
    ...
    def _export_csv(self) -> None:
        if not self.rec_rows:
            ...
            return
        default = datetime.now().strftime("tle5012b_rpm_%Y%m%d_%H%M%S.csv")
    ...
    def _on_sample(self, sample: tuple) -> None:
        ...
        if elapsed >= 1.0:
            self.root.title(
                f"TLE5012B 监视器 | 采样{self.sample_hz}Hz→控制{self.ctrl_hz}Hz | 收{self._rx_count / elapsed:.0f}Hz"
            )
            self._rx_count = 0
            self._rx_t0 = time.time()
        ...
        warns = []
        if ef:
            warns.append("SSC状态异常")
        if mag_l:
            warns.append("角度无效(NO_GMR)")
        if mag_h:
            warns.append("MAGH")
        agc_txt = f"AGC={agc}" if agc >= 0 else "AGC=N/A"
        self.diag_var.set(
