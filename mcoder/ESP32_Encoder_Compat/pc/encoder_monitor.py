...
# -*- coding: utf-8 -*-
"""
磁编码器兼容测试 PC 监视器（AS5047P / TLE5012B）
- 实时显示角度 0~360° / 0~2π，钟表式相位表盘（0° 在上方，顺时针）
- 下拉选择传感器，经串口发送 S0/S1 切换固件驱动
- 开始/停止记录，绘制转速(RPM) vs 时间，导出 CSV
"""
...
BAUD = 921600  # 与固件一致；400Hz 行输出需要 >115200
DEFAULT_PORT = "COM18"
AUTO_CONNECT = True  # 启动时若有默认口则自动连接
LIVE_WINDOW_S = 10.0  # 实时转速曲线窗口长度（秒）
UI_REFRESH_MS = 40  # 表盘/曲线刷新约 25 Hz；数据仍按 400 Hz 全收
MAX_REC_POINTS = 120000  # 400Hz ≈ 5 分钟；防止长时间记录撑爆内存
EXPECTED_CTRL_HZ = 400
EXPECTED_SAMPLE_HZ = 1600
DEFAULT_SENSOR = "TLE5012B"  # 或 "AS5047P"
SENSOR_CMDS = {
    "AS5047P": "S0",
    "TLE5012B": "S1",
}
...
def parse_meta_line(line: str) -> dict | None:
    """解析固件注释，如 # sample_hz=1600 ctrl_hz=400 ..."""
    line = line.strip()
    if not line.startswith("#"):
        return None
    meta: dict = {}
    for key in ("sample_hz", "ctrl_hz", "oversample", "baud"):
        token = f"{key}="
...
def parse_line(line: str) -> tuple | None:
    """解析: t_ms,raw,deg,rad,rpm[,ef,agc,magL,magH[,rate_hz]]"""
    line = line.strip()
    ...
    try:
        ...
        rate_hz = int(float(parts[9])) if len(parts) >= 10 else EXPECTED_CTRL_HZ
        return t_ms, raw, deg, rad, rpm, ef, agc, mag_l, mag_h, rate_hz
    except ValueError:
        return None
...
class SerialReader(threading.Thread):
    def __init__(
        ...
        self.cmd_q = cmd_q if cmd_q is not None else queue.Queue()
...
class EncoderMonitorApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("磁编码器兼容监视器 (AS5047P / TLE5012B)")
        ...
        self.sample_hz = EXPECTED_SAMPLE_HZ
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="传感器").pack(side=tk.LEFT, padx=(12, 0))
        self.sensor_var = tk.StringVar(value=DEFAULT_SENSOR)
        self.sensor_cb = ttk.Combobox(
            top,
            textvariable=self.sensor_var,
            values=list(SENSOR_CMDS.keys()),
            width=12,
            state="readonly",
        )
        self.sensor_cb.pack(side=tk.LEFT, padx=(4, 4))
        ttk.Button(top, text="切换", command=self._apply_sensor).pack(side=tk.LEFT)
    ...
    def _connect(self) -> None:
        ...
        while not self.cmd_q.empty():
            try:
                ...
                break
        self.reader = SerialReader(port, self.q, self.stop_evt, self.cmd_q)
        self.reader.start()
        self.btn_connect.configure(text="断开")
        self.btn_rec.configure(state=tk.NORMAL)
        sensor = self.sensor_var.get()
        self.status_var.set(
            f"已连接 {port} @ {BAUD} | {sensor} | 采样 {self.sample_hz}Hz → 控制 {self.ctrl_hz}Hz"
        )
        self.prev_deg = None
        self.prev_t_ms = None
        self._rx_count = 0
        self._rx_t0 = time.time()
        # 连接后稍等再发切换命令，避免与固件启动横幅抢字节
        self.root.after(300, lambda: self._apply_sensor(silent=True))

    def _apply_sensor(self, silent: bool = False) -> None:
        name = self.sensor_var.get().strip()
        cmd = SENSOR_CMDS.get(name)
        if not cmd:
            if not silent:
                messagebox.showwarning("提示", "未知传感器")
            return
        if not (self.reader and self.reader.is_alive()):
            if not silent:
                messagebox.showinfo("提示", "请先连接串口后再切换传感器")
            return
        self.cmd_q.put(cmd)
        self.prev_deg = None
        self.prev_t_ms = None
        port = self.port_var.get().strip()
        self.status_var.set(
            f"已连接 {port} @ {BAUD} | {name} ({cmd}) | 采样 {self.sample_hz}Hz → 控制 {self.ctrl_hz}Hz"
        )
    ...
    def _export_csv(self) -> None:
        if not self.rec_rows:
            ...
            return
        sensor = self.sensor_var.get().replace(" ", "_").lower() or "encoder"
        default = datetime.now().strftime(f"{sensor}_rpm_%Y%m%d_%H%M%S.csv")
            ...
            while True:
                ...
                if isinstance(item, tuple) and item and item[0] == "__meta__":
                    ...
                    if isinstance(meta, dict):
                        ...
                        if "sample_hz" in meta:
                            self.sample_hz = int(meta["sample_hz"])
                        port = self.port_var.get().strip()
                        sensor = self.sensor_var.get()
                        self.status_var.set(
                            f"已连接 {port} @ {BAUD} | {sensor} | 采样 {self.sample_hz}Hz → 控制 {self.ctrl_hz}Hz"
    ...
    def _on_sample(self, sample: tuple) -> None:
        t_ms, raw, deg, rad, rpm_fw, ef, agc, mag_l, mag_h, rate_hz = sample
        if rate_hz > 0:
            self.ctrl_hz = rate_hz

        self._rx_count += 1
        elapsed = time.time() - self._rx_t0
        if elapsed >= 1.0:
            self.root.title(
                f"兼容监视器 | 采样{self.sample_hz}Hz→控制{self.ctrl_hz}Hz | 收{self._rx_count / elapsed:.0f}Hz"
            )
            self._rx_count = 0
            self._rx_t0 = time.time()
        ...
        if self.recording:
            ...
            if len(self.rec_rows) >= MAX_REC_POINTS:
                ...
                messagebox.showinfo(
                    "提示",
                    f"记录已达上限 {MAX_REC_POINTS} 点（约 {MAX_REC_POINTS / self.ctrl_hz:.0f}s @ {self.ctrl_hz}Hz），已自动停止",
                )
                return
