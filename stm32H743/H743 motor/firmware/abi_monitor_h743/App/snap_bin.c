...
const SnapPt *Snap_Points(void) { return snap; }

/* ---------- zlib CRC32（IEEE 多项式，PC 工具 snap_crc32 对齐） ---------- */
...
uint32_t Snap_Crc32(void)
{
    uint32_t crc = 0xFFFFFFFFu;
    const uint8_t *p = (const uint8_t *)snap;
    for (uint32_t i = 0; i < snap_count * sizeof(SnapPt); i++)
        crc = (crc >> 8) ^ crc_table[(crc ^ p[i]) & 0xFFu];
