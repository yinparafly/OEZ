/**
  ...
  * @brief   STM32H743xx vector table for GNU ARM toolchain
  *          Vector table order matches the MDK version (166 entries).
  ******************************************************************************
  */
  .syntax unified
  .cpu cortex-m7
 ...
 *         necessary set is performed, after that a call to the application
 *         entry point is made.
 * @param  None
 * @retval : None
*/
...
/* Copy the data segment initializers into the .data segment */
  ldr r0, =_sdata
  ldr r1, =_edata
  ldr r2, =_sidata
  movs r3, #0
...
* must be placed on this table to ensure that it ends up at physical address
* 0x0000.0000.
*
*******************************************************************************/
...
g_pfnVectors:
  ...
  /* External Interrupts */
  .word     WWDG_IRQHandler                   /* Window WatchDog interrupt (wwdg1_it)          */
  ...
  .word     TIM8_TRG_COM_TIM14_IRQHandler     /* TIM8 Trigger and Commutation Interrupt and TIM14 global interrupt */
  ...
  .word     HRTIM1_Master_IRQHandler          /* HRTIM Master Timer global Interrupts            */
